...
test('REGRESSION: ensure we\'re receiving remote handshake', function(t){
  ...
  var mux1 = multiplexer(encryptionKey, { userData: { foo: 'bar' } })
  var mux2 = multiplexer(encryptionKey, { userData: { foo: 'quux' } })

  mux1.once('ready', function (header) {
    t.equal(header.id, mux2._id)
  })
  mux2.once('ready', function (header) {
    t.equal(header.id, mux1._id)
  })
